...
function alldone_update(){update_notify_count=0;var prep_data={todo_list:todo_list,colourscheme:user_options.colourscheme};var obj={'request':'update','session':login.session,'uid':login.uid,'owner':todo_owner,'uri':todo_title,'data':JSON.stringify(prep_data)};var jqxhr=$.post(uri,obj).done(function(data){$('#updating').hide();var json=jQuery.parseJSON(data);if(json.result===0){if(json.error==='login'){if(runtime==='app'){login={};app_boot();}else{alert('please login again!');}}}}).fail(function(){dialog({type:'error',title:'Error saving',text:'Error saving data.'});$('#updating').hide();});}
...
if(val===targetid){todo_list=recursive_magic(insert_bc,0,'add',dragged_item,todo_list);}}});todo_list=recursive_tidy(todo_list);open_breadcrumbs=breadcrumb_removelast(insert_bc);redraw(open_breadcrumbs);alldone_update_notify();}var current_hover=0,label_mousedown=false;function sizeup(){var scale_by=screenmode();var newWidth=($(window).width()/scale_by);$('ul').outerWidth(newWidth);var count=$('ul').length;$('#wrapper').width(count*newWidth);var inputWidth=newWidth-100;$('input.edit, input.new').width(inputWidth-50);$('textarea.edit, textarea.new').width(inputWidth);$('li label').width(inputWidth);}
...
$('body').addClass(user_options.colourscheme);redrawing=breadcrumbs;old_breadcrumbs=open_breadcrumbs;retain_forms(false);retain_scroll(false);html+=draw_list(working_breadcrumbs);$.each(breadcrumbs,function(i,val){working_breadcrumbs.push(breadcrumbs[i]);html+=draw_list(working_breadcrumbs);});if((breadcrumbs.length<open_breadcrumbs.length)&&(open_breadcrumbs.length>(screenmode()-1))){open_breadcrumbs=breadcrumbs;updateScroll(false,breadcrumbs.length,html);}else{open_breadcrumbs=breadcrumbs;redraw_finalize(html,true);}}}
...
$(window).resize(function(){sizeup();});var todo_list=[],open_breadcrumbs=[],old_breadcrumbs=[],form_history=[],scroll_history=[],form_count=0;var recursive_fetched={},open_edit=false,sort_mode=false,pause_redraw=false,custom_context=true;var user_options={fullscreen:false,colourscheme:'slate',editmode:false,};var session=0,this_user=-1;var first_load=false,thar_be_magic=false,load_from_var=false,public_use=false,read_only=false;var callbacks=[],redrawing=[];var todo_blank={title:'',unique:0,tags:{},details:'',done:false,icon:'',dateSet:0,dateDue:0,dateComplete:0,children:[]};var todo_title="list",todo_owner='';var todo_list=[{title:'Debugging',done:false},{title:'willy',done:false},{title:'Marketing',done:false,children:[{title:'willy',done:false},{title:'poo',done:false,children:[{title:'wank',done:false},{title:'hat',done:false,children:[{title:'wank',done:false},{title:'aaaaaa',done:false},{title:'afff',done:false}]},{title:'afff',done:false}]}]},{title:'Willy',done:false},{title:'Flannageddon',done:false,children:[{title:'assssss',done:false},{title:'moooma',done:false}]}];var todo_list=[{"title":"1","done":false,"subtext":"","tags":["wank","ham","spam"],"icon":"","dateSet":0,"dateDue":0,"dateComplete":0,"children":[{"title":"a","done":false,"subtext":"","icon":"","dateSet":0,"dateDue":0,"dateComplete":0,"children":[]},{"title":"b","done":false,"subtext":"","icon":"","dateSet":0,"dateDue":0,"dateComplete":0,"children":[]},{"title":"c","done":false,"subtext":"","icon":"","dateSet":0,"dateDue":0,"dateComplete":0,"children":[{"title":"arghhh#]","done":false,"subtext":"","icon":"","dateSet":0,"dateDue":0,"dateComplete":0,"children":[]}]}]},{"title":"blah blah rarh hello blh lagh ah asjk ash asjkdh asjkdhas jkasdhasdjka asdhasasd","done":false,"subtext":"","icon":"","dateSet":0,"dateDue":0,"dateComplete":0,"children":[]},{"title":"3","done":false,"subtext":"","icon":"","magic":{"list":"no_github_issues","username":"rails","repo":"rails"},"dateSet":0,"dateDue":0,"dateComplete":0,"children":[]}];function callback(set,what){if(set){callbacks.push(what);}else{if(callbacks.length>0){$.each(callbacks,function(key,value){if(value.method==='focus_new'){setTimeout(function(){$('#'+value.id+' textarea.new').val('');$('#'+value.id+' textarea.new').focus();},1);}});callbacks=[];}}}
function draw_list(crumbs){var drawList=get_breadcrumbs(crumbs),crumbIndex=crumbs.length;var text_bc=textualize_breadcrumbs(crumbs);var html="<ul class=\"list\" id=\"crumbs-"+(crumbIndex)+"\" breadcrumbs=\""+text_bc+"\">";if(crumbIndex===0){html+="<li class=\"static list_header\">";if(runtime=='app'){html+="<div class=\"mainmenu app\" title=\"load lists or log out\"><i class=\"fa fa-bars\"></i></div>";}
...
html+="</li>";}
if(!read_only){html+="<li class=\"static\"><div class=\"checkbox disabled\">"+checkbox_please(false)+"</div> <textarea class=\"new\" type=\"text\" id=\"new-"+crumbIndex+"\" breadcrumbs=\""+text_bc+"\" rows=1></textarea></li>";}
html+="</ul>";return html;}
