...
return elements;};}}));var login={};if(localmode){var uri='http://'+localmode_ip+'/alldone.io/api/';var homeuri='http://'+localmode_ip+'/alldone.io/';}else{var uri='http://www.alldone.io/api/';var homeuri='http://www.alldone.io/';}
var update_notify_count=0;var runtime='none';function alldone_login(user,pswd){var obj={'request':'login','user':user,'pswd':pswd};$('#updating').show();var jqxhr=$.post(uri,obj).done(function(data){app_login_complete(jQuery.parseJSON(data));$('#updating').hide();}).fail(function(xhr,textStatus,errorThrown){$('#updating').hide();alert(xhr.responseText);alert(textStatus);});}
...
function alldone_update(){update_notify_count=0;var prep_data={todo_list:todo_list,colourscheme:user_options.colourscheme,mode:user_options.mode};var obj={'request':'update','session':login.session,'uid':login.uid,'owner':todo_owner,'uri':todo_title,'data':JSON.stringify(prep_data)};var jqxhr=$.post(uri,obj).done(function(data){$('#updating').hide();var json=jQuery.parseJSON(data);if(json.result===0){if(json.error==='login'){if(runtime==='app'){login={};app_boot();}else{alert('please login again!');}}}}).fail(function(){dialog({type:'error',title:'Error saving',text:'Error saving data.'});$('#updating').hide();});}
...
if(val===targetid){todo_list=recursive_magic(insert_bc,0,'add',dragged_item,todo_list);}}});todo_list=recursive_tidy(todo_list);open_breadcrumbs=breadcrumb_removelast(insert_bc);redraw(open_breadcrumbs);alldone_update_notify();}var current_hover=0,label_mousedown=false,clickcount=0;var blahblahblah=false;function sizeup(){var scale_by=screenmode();if(user_options.mode!=='vertical'){var newWidth=($(window).width()/scale_by);$('ul').outerWidth(newWidth);var count=$('ul').length;$('#wrapper').width(count*newWidth);if($('body').hasClass('vertical')){$('body').removeClass('vertical');}
...
$('textarea.edit, textarea.new').width(newWidth-130);for(i=1;i<6;i++){$('ul.crumbs-'+i+' textarea.edit, ul.crumbs-'+i+' textarea.new').width(newWidth-130-(vert_indent*i));$('ul.crumbs-'+i+' li label').width(newWidth-130-(vert_indent*i));}}}
...
if((user_options.editmode)&&(!read_only)){$('li label').click(function(){modify(this);});}else{$('li label').not('li.open label, li.static').on("mousedown touchstart",function(){var d=new Date();clickcount=d.getTime();});$('li label').not('li.open label, li.static').on("mouseup touchend",function(){var d=new Date();if((d.getTime()-clickcount)>450){modify(this);}else{expand($(this).parent());}
clickcount=0;});$('li.open label').not('li.static').on("mousedown touchstart",function(){var d=new Date();clickcount=d.getTime();});$('li.open label').not('li.static').on("mouseup touchend",function(){var d=new Date();if((d.getTime()-clickcount)>450){modify(this);}else{close($(this).parent());}
clickcount=0;});$('li').not('li.open, li.static').click(function(){expand($(this));}).children().click(function(e){return false;});$('li.open').not('li.static').click(function(){close($(this));}).children().click(function(e){return false;});$('.edit_button').click(function(){modify(this);});$('li .count').not('li.open .count').click(function(){expand($(this).parent());});$('li.open .count').click(function(){close($(this).parent());});}
...
function retain_forms(restore){if(!restore){form_history=[];form_count=$('li textarea.new').length;$('li textarea.new').each(function(){form_history[$(this).attr("breadcrumbs")]={};form_history[$(this).attr("breadcrumbs")].title=$(this).val();});}
if((restore)&&(form_history)){var lastinput=0;$('li textarea.new').each(function(){if(form_history[$(this).attr("breadcrumbs")]){var res=form_history[$(this).attr("breadcrumbs")];$(this).val(res.title);lastinput=this;}});}}
...
$(window).resize(function(){sizeup();});var todo_list=[],open_breadcrumbs=[],old_breadcrumbs=[],form_history=[],scroll_history=[],form_count=0;var multi_crumbs=[],multi_crumbs_text=[];var recursive_fetched={},open_edit=false,sort_mode=false,pause_redraw=false,custom_context=true;var user_options={fullscreen:false,colourscheme:'slate',editmode:false,};var vert_indent=40;var session=0,this_user=-1;var first_load=false,thar_be_magic=false,load_from_var=false,public_use=false,read_only=false;var callbacks=[],redrawing=[];var todo_blank={title:'',unique:0,tags:{},details:'',done:false,icon:'',dateSet:0,dateDue:0,dateComplete:0,children:[]};var todo_title="list",todo_owner='';var todo_list=[];function callback(set,what){if(set){callbacks.push(what);}else{if(callbacks.length>0){$.each(callbacks,function(key,value){if(value.method==='focus_new'){setTimeout(function(){$('ul[breadcrumbs="'+value.id+'"] textarea.new').val('');$('ul[breadcrumbs="'+value.id+'"] textarea.new').focus();},1);}});callbacks=[];}}}
function draw_list(crumbs){var drawList=get_breadcrumbs(crumbs),crumbIndex=crumbs.length;var text_bc=textualize_breadcrumbs(crumbs);match_isopen_cache();var ulClass='horizontal',ulStyle='';if(user_options.mode==='vertical'){ulClass='vertical';ulStyle='margin-left: '+(vert_indent*crumbIndex)+'px;';}
...
for(i=0;i<drawList.length;i++){var item=drawList[i];var sublist=[];if(user_options.mode=='vertical'){sublist=match_sublist(multi_crumbs,i);}
html+=draw_list_item(item,crumbIndex,i,text_bc,crumbs);if(user_options.mode=='vertical'){var drawSubList=false;if(isNumber(redrawing[crumbIndex])){if((redrawing[crumbIndex]===i)){}}
...
function draw_list_vertical_sub(item,thisIndex,crumbIndex,text_bc,crumbs,sublist){var crumbIndexIncrem=crumbIndex+1,text_bcIncrem=text_bc+'-'+thisIndex,countIndex=0;var html="<li id=\"vert-crumbs-"+(crumbIndexIncrem)+"\">";html+="<ul class=\"list vertical crumbs-"+(crumbIndexIncrem)+"\" style=\"margin-left: "+(vert_indent)+"px;\" id=\"crumbs-"+(crumbIndexIncrem)+"\" breadcrumbs=\""+text_bcIncrem+"\">";$.each(item.children,function(key,value){html+=draw_list_item(this,crumbIndexIncrem,countIndex,text_bcIncrem,crumbs);var new_sublist=match_sublist(sublist,countIndex),drawSubList=false;if(isNumber(redrawing[crumbIndexIncrem])){if(redrawing[crumbIndexIncrem]===countIndex){}}
...
function match_sublist(multi,match){var multi_return=[],hit=false;$.each(multi,function(i,val){if(((val[0])||(parseInt(val[0])===0))&&(val[0]==match)){var n=val.slice();n.splice(0,1);multi_return.push(n);hit=true;}});return{list:multi_return,hit:hit};}
function match_isopen_cache(){multi_crumbs_text=[];$.each(multi_crumbs,function(i,val){multi_crumbs_text.push(textualize_breadcrumbs(val));});}
function match_isopen(text_bc){var match=false;$.each(multi_crumbs_text,function(i,val){if(text_bc===val){match=true;}});return match;}
...
function expand(e){var bc=breadcrumb_explosion($(e).attr("breadcrumbs"));if(user_options.mode==='vertical'){multi_crumbs.push(bc);}
redraw(bc);}
function close(e){var bc=breadcrumb_explosion($(e).attr("breadcrumbs"));if(user_options.mode==='vertical'){close_multi(bc);}else{bc.splice(-1,1);multi_crumbs=[];}
redraw(bc);}
function close_multi(bc){multi_crumbs=$.grep(multi_crumbs,function(el,i){var match=true;for(i=0;i<bc.length;i++){if((el[i])||(parseInt(el[i])===0)){if(bc[i]!==el[i]){match=false;}}else{match=false;}}
if(match){return false;}
return true;});}
function array_equal(a,b){var is_same=(a.length==b.length)&&a.every(function(element,index){return element===b[index];});return is_same;}
...
return n;}var cached_lists=[];runtime='app';$(window).load(function(){app_boot();});function app_boot(){if((login)&&(login.session)&&(login.uid)){if(!login.always_open){app_lists(false);}else{alldone_grab(login.always_open);}}else{app_login();}}
function app_login(){dialog({type:'login'});$('#dialog_login').click(function(){alldone_login($('#login_email').val(),$('#login_password').val());});$('#dialog_signup').click(function(){alert('[beta] please go to http://www.alldone.io/ to sign up :)');});}
function app_login_complete(data){if(data){if(data.result===0){dialog_close();dialog({type:'error',title:'Login',text:'Login failed. Please try again!'});$('#dialog_okay').click(function(){dialog_close();app_login();});}else{login.session=data.session;login.uid=data.uid;update_session();dialog_close();app_boot();}}else{alert(data);}}
function login_dialog(){var html="";html+='<div class=fieldset>';html+='<label>Email</label>';html+='<input type=email id=login_email value="">';html+='</div>';html+='<div class=fieldset>';html+='<label>Password</label>';html+='<input type=password id=login_password value=>';html+='</div>';return{html:html};}
function app_lists(allow_close){var obj={'request':'lists','session':login.session,'uid':login.uid};$('#updating').show();var jqxhr=$.post(uri,obj).done(function(data){var json=jQuery.parseJSON(data);if(json.result===1){cached_lists=json.data;dialog({type:'lists',allow_close:allow_close});$('#dialog_select').click(function(){lists_dialog_select();});$('#dialog_create').click(function(){alert('[beta] please go to http://www.alldone.io/ to create lists');});$('#dialog_logout').click(function(){dialog_close();login={};update_session();app_boot();});}else{if(json.error==='login'){login={};app_boot();}
dialog({type:'error',title:'Error',text:'Unable to fetch lists. Click okay to try again.'});$('#dialog_okay').click(function(){dialog_close();app_boot();});}
$('#updating').hide();}).fail(function(){$('#updating').hide();alert('error connecting to server');});}
function lists_dialog(){var html="<select size=10 id=dialog_list>";$.each(cached_lists,function(key,value){html+="<option value=\""+value.username+"/"+value.uri+"\">/"+value.username+"/"+value.uri+"</option>";});html+="</select>"
return{html:html};}
function lists_dialog_select(){var load=$('#dialog_list').val();if(load!==""){alldone_grab(load);if($("#alwaysopen").is(':checked')){login.always_open=load;}else{login.always_open=false;}
update_session();}}
function app_grab_complete(data){if(data){if(data.result===1){public_use=false
read_only=false;todo_data=jQuery.parseJSON(data.list_data);if((load_permission==8)||(load_permission==9)||(load_permission==1)){public_use=true;}
if((load_permission==8)||(load_permission==1)){read_only=true;}
todo_title=data.uri;todo_owner=data.owner;todo_list=todo_data.todo_list;user_options.colourscheme=todo_data.colourscheme;first_load=true;redraw([]);dialog_close();}else{dialog({type:'error',title:'Error',text:'Unable to load that list. Please try again or contact alldone.io support.'});$('#dialog_okay').click(function(){dialog_close();app_boot();});}}}
function app_window_update(){window.title="/"+todo_owner+"/"+todo_title;}