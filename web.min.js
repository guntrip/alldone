...
var id=$(e).parent().attr("id");var id_split=id.split("-");var bc_location=id_split[1];var bc_index=id_split[2];var bc=breadcrumb_explosion($(e).parent().attr("breadcrumbs"));open_edit=e;modify_todo({editing:true,editingUpdate:true},bc);$('body').click(function(event){console.log('i tried');var saveChanges=false;if(!$(event.target).closest('#'+id).length){modify_save_open();}});$('.editing_force').click(function(){modify_save_open();});$('#'+id+' textarea.edit').focus();$('#'+id+' textarea.edit').select();}}
...
todo_list=load_item.todo_list;todo_title=load_title;todo_owner=load_owner;user_options.colourscheme=load_item.colourscheme;first_load=true;redraw([]);}else{dialog({type:'error',title:'Error',text:'Data not present. Please retry.'});}});function get_cookie(){var coo=document.cookie.split(';'),r=0;$.each(coo,function(key,value){var trimmed=$.trim(value);var t=trimmed.split('=');if(t[0]==='alldone'){var s=decodeURIComponent(t[1]);r=s.split('^');r=r[1];}});return r;}